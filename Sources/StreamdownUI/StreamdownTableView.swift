...
struct StreamdownTableView: View {
    ...
    private func copyTable(format: TableFormat) {
    ...
    private func downloadTable(format: TableFormat) {
        guard let text = formattedTable(format: format) else { return }
        let filename = format == .csv ? "table.csv" : "table.md"
        shareFile(text, filename: filename)
    }

    private func formattedTable(format: TableFormat) -> String? {
...
private struct StreamdownTableCell: View {
    let text: String
    ...
    var body: some View {
        if let inlineContent {
            Text(inlineContent.attributed)
                ...
                .frame(
                    maxWidth: .infinity,
                    minHeight: StreamdownTableStyle.rowMinHeight,
...
private struct StreamdownTableFullscreen: View {
    let headers: [String]
    let rows: [[String]]
    let controls: StreamdownControls.Table
